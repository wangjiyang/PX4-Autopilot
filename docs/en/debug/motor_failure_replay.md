...
The offset is where the fitted line reaches zero command, which is outside the range a flying vehicle ever uses, so its sign depends on how curved that vehicle's current-vs-command relation is.
It came out negative above; on a vehicle whose relation is closer to a straight line it comes out positive, and no clamping is needed:

```text
# 4 logs, 123659 armed samples
# I_expected = 6.58*u + 0.44 A   (residual sigma 0.51 A)
# mean command 0.465, mean current 3.50 A

MOTFAIL_C2T 6.58
MOTFAIL_IDLE 0.44
```

Either way, set what the tool prints, with a negative offset clamped to 0.
